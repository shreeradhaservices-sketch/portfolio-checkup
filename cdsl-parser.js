...
function parseMfFolios(rawText) {
  const chunks = stripBoilerplate(rawText).split(/(?=AMC Name\s*:)/);
  const folios = [];
...
function parseSchemeValues(rawText) {
  const sectionMatch = stripBoilerplate(rawText).match(/MUTUAL FUND UNITS HELD AS ON[\s\S]*?(?=Load Structures|NOTES TO CAS|$)/);
 ...
 */
function buildUnifiedBroadCategory(mfBroad, demat, officialAllocation) {
  const totals = {};
  for (const b of mfBroad) totals[b.category] = (totals[b.category] || 0) + b.value;
  totals['Equity'] = (totals['Equity'] || 0) + demat.directEquityValue + demat.dematFundUnitsValue;
  ...
  // Anchor Preference Shares and any residual "Others" to CDSL's own
  // official asset-class table, since we don't independently parse
  // preference share holdings — this keeps the broad total traceable to
  // an authoritative source rather than silently omitting it.
  const findOfficial = (name) => {
    const match = (officialAllocation || []).find(a => a.category === name);
    return match ? match.value : 0;
  };
  const officialPreference = findOfficial('Preference Shares');
  if (officialPreference > 0) totals['Preference Shares'] = (totals['Preference Shares'] || 0) + officialPreference;
...
function buildCdslSummary(rawText) {
  ...
  const broadCategory = buildUnifiedBroadCategory(mfCategoryBreakdown.broad, demat, allocation);
